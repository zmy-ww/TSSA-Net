...
class BrainNetworkTransformer_mdd(BaseModel):
    ...
    def forward(self,
        ...
        pos = self.pos_encoding_mlp(semantic_similarity_matrix)
        all_outputs = []
        ...
        # 对静态进行时空注意力编码
        x = node_feature[i]
        edge_index, edge_weight = self.adj_to_edge_index(node_feature[i])


        if self.pos_encoding == 'identity':
            if self.prompt_tokens is not None:
                pos_emb = self.prompt_tokens.unsqueeze(0).unsqueeze(0).expand(bz, time_steps, *pos.shape)
        ...
        # time_weights 表示每个样本中的每个节点在不同时间步上的权重 形状是 [batch_size, time_steps, num_nodes]
        temporal_output, time_weights = self.time_attention(temporal_output)
        # 进行聚类操作
        ...
        if self.pos_encoding == 'identity':
            if self.prompt_tokens is not None:
                pos_emb = self.prompt_tokens.unsqueeze(0).expand(bz, *pos.shape)
        ...
        # 计算动态和静态特征之间的相似度
        similarity = compute_similarity(final_repr, node_feature_static)

        # 计算对比损失
        contra_loss = contrastive_loss(similarity)



        # 开始聚类
        node_feature_static, assignment_static = self.static_trans_pooling2(node_feature_static) # 带聚类的
        node_feature_dynamic, assignment_dynamic = self.final_trans_pooling(final_repr)
        node_feature_dynamic = self.dim_reduction(node_feature_dynamic)
